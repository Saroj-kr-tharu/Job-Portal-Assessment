...
const fieldClass =
  "w-full px-3 py-2 rounded-[10px] border border-[#2e3548] bg-[#1e2330] text-[14px] text-[#e8ecf4] placeholder-[#555f72] focus:outline-none focus:border-[#3b82f6]/60 transition-colors";
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="w-full max-w-lg bg-[#1a1f2e] border border-[#2e3548] rounded-[16px] shadow-2xl flex flex-col max-h-[90vh]">
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#2e3548] flex-shrink-0">
            <div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col overflow-y-auto scrollbar-thin">
            <div className="px-5 py-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  type="date"
                  className={`${fieldClass} ${errors.applied_date ? "border-[#dc2626]" : ""}`}
                />
            ...
            {/* Footer */}
            <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-[#2e3548] flex-shrink-0">
              <button
                type="button"
                onClick={onClose}
                disabled={isLoading}
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="flex items-center gap-2 px-5 py-2 rounded-[10px] bg-[#1a3a5c] border border-[#2563eb]/40 text-[#60a5fa] text-[14px] font-semibold hover:bg-[#1e4a75] transition-colors disabled:opacity-50"
              >
                {isLoading && (
