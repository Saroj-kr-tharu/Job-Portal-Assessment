...
export default function Navbar({ currentPage, onNavigate }: Props) {
  ...
  return (
    <nav className="h-16 flex items-center px-5 border-b border-[#2e3548] bg-[#1a1f2e] z-30 relative flex-shrink-0">
      {/* Logo */}
      <button
        onClick={() => onNavigate("home")}
        className="flex items-center gap-2.5 mr-8 group"
      >
        <div className="w-8 h-8 rounded-[8px] bg-[#1a3a5c] border border-[#2563eb]/40 flex items-center justify-center">
          <FaBriefcase size={14} className="text-[#60a5fa]" />
        </div>
        <span className="text-[16px] font-bold text-[#e8ecf4] tracking-tight">
          Inter<span className="text-[#60a5fa]">Sathi</span>
        </span>
      </button>
      ...
      <div className="ml-auto flex items-center gap-2">
        <button
          onClick={() => onNavigate("tracker")}
          className="hidden sm:flex items-center gap-1.5 px-3.5 py-1.5 rounded-[8px] bg-[#1a3a5c] border border-[#2563eb]/40 text-[#60a5fa] text-[13px] font-semibold hover:bg-[#1e4a75] transition-colors"
        >
          Open Tracker ↗
        </button>
        ...
        {/* Mobile menu toggle */}
        <button
          className="sm:hidden w-8 h-8 flex items-center justify-center rounded-[8px] border border-[#2e3548] text-[#555f72]"
      ...
      {menuOpen && (
        <div className="absolute top-full left-0 right-0 bg-[#1a1f2e] border-b border-[#2e3548] px-4 py-3 flex flex-col gap-2 sm:hidden z-50">
          <button
            ...
            className={`px-3 py-2 rounded-[8px] text-[14px] font-medium text-left ${
